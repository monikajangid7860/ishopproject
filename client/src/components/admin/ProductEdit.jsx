...
const ProductEdit=({ productId })=>{
  const [form,setForm]=useState({name:"",slug:"",original_price:"",final_price:"",discount_percentage:""});
  ...
  const fetchProduct=async()=>{
    try{
      ...
      setSelectedCategory({value:p.category_id._id,label:p.category_id.name});
      setSelectedBrand({value:p.brand_id._id,label:p.brand_id.name});
      const colors=p.color_ids.map(c=>({value:c._id,label:c.name}));
  ...
  const handleNameChange=(e)=>{
    ...
    setForm({...form,name,slug:slugGenerator(name)});
  };
  ...
  const handlePriceChange=(key,value)=>{
    ...
    if(op&&fp&&fp<=op){
      updated.discount_percentage=(100-(fp/op)*100).toFixed(2);
    }else{
  ...
  const submitHandler=async(e)=>{
    ...
    if(colorIds.length)formData.append("color_ids",JSON.stringify(colorIds));
    if(mainImage?.file)formData.append("thumbnail",mainImage.file);
    try{
      const res=await axiosApiInstance.put(`product/update/${productId}`,formData,{headers:{"Content-Type":"multipart/form-data"}});
      res.data?.flag===1?notify("Product updated successfully",1):notify(res.data?.message||"Update failed",0);
    }catch(err){
      console.error(err);
      ...
      <form onSubmit={submitHandler} className="space-y-8">
        <div className="bg-white border border-gray-200 rounded-xl p-6">
          ...
          <div className="grid grid-cols-2 gap-5">
            <input value={form.name} onChange={handleNameChange} className="admin-input"/>
            <input value={form.slug} readOnly className="admin-input bg-gray-50"/>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6">
          ...
          <div className="grid grid-cols-3 gap-5">
            <input type="number" value={form.original_price} onChange={(e)=>handlePriceChange("original_price",e.target.value)} className="admin-input"/>
            <input type="number" value={form.final_price} onChange={(e)=>handlePriceChange("final_price",e.target.value)} className="admin-input"/>
            <input value={form.discount_percentage} readOnly className="admin-input bg-gray-50"/>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6">
          ...
          <div className="grid grid-cols-3 gap-5">
            <Select value={selectedCategory} onChange={setSelectedCategory} options={categoryData.map(c=>({value:c._id,label:c.name}))}/>
            <Select isMulti value={selectedColors} onChange={colorChangeHandler} options={colorData.map(c=>({value:c._id,label:c.name}))}/>
            <Select value={selectedBrand} onChange={setSelectedBrand} options={brandData.map(b=>({value:b._id,label:b.name}))}/>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6">
          ...
          <FileUpload onFilesChange={(files)=>setMainImage(files[0])} maxFiles={1}/>
        </div>

        <div className="flex justify-end pt-4 border-t">
          <button className="px-6 py-2.5 text-sm font-medium text-white bg-gray-900 hover:bg-gray-800 rounded-lg">
            Update Product
          </button>
        </div>
      </form>
